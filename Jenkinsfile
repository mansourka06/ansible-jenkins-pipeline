    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                ...
                git credentialsId: 'your-git-credentials-id', url: 'https://github.com/yourusername/your-repo.git'
            }
        }
        ...
        stage('Test and deploy the application') {
            ...
            stages {
               ...
               stage("Cleanup App Deployment") {
                    when {
                       expression { GIT_BRANCH == 'origin/test' }
                    }
                   steps {
                       sh '''
                       ansible-playbook  -i hosts.yml --vault-password-file vault.key  --extra-vars "ansible_sudo_pass=$SUDOPASS" cleanup.yml
                       '''
                   }
               }  
            }
